...
# Clean (adjustments) Your Data with Help from AI
...
# Open file with API key and use the read func to read -t.
...
# Check if cleaned dataset exists and load it if available. If so, use the open func to open it. `rb` = result back. Read it as `f` which in our case stands for file.
if os.path.exists("cleaned_data_final.pkl"):
    ...
    # Load-in our raw data - revenue and expenses xlsx file
    ...
    if not user_prompt.strip():
        ...
        with st.spinner("Generating cleaning code…"):
            try:
                ...
                st.session_state.history.append(("Bot", "Cleaning code generated successfully. Use the button above to apply and save it if you wish."))
...
if st.session_state.latest_code:
    if st.button("Apply & Save Cleaning Change"):
        try:
            ...
            # Also save cleaned dataset to the Chapter_4 folder one level up - `cwd` = current working directory.
            hotel_data_dashboard = os.path.join(os.path.dirname(os.getcwd()), "clean_data", "cleaned_data_final.pkl")
            # Ensure this path exists
            os.makedirs(os.path.dirname(hotel_data_dashboard), exist_ok=True)  
            # Save current dataframe to a pickle file for use by temp script. `wb` = open the file in write-binary mode.
            with open(hotel_data_dashboard, "wb") as f:
                pickle.dump(df, f)

            # Create temporary Python script containing AI-generated cleaning code. `rb` = result back. `f"""` = formatted string literal.
            temp_code = f"""
import pickle
import os
...
st.download_button(
    ...
    mime="text/csv" # The MIME type telling the browser this is a CSV file.
)

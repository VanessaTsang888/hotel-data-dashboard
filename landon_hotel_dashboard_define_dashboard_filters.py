...
import pandas as pd
# This module in Python is used for serialising and deserialising Python obj's, allowing data to be saved to or loaded from a file.
# When using `pickle` with `os`, caution is required due to security vulnerabilities.
import os, pickle
from openai import OpenAI
# Conveention to access NumPy functions and objects using the shorter np. prefix, such as np.array() or np.sum()
import numpy as np
...
if not os.path.exists("cleaned_data_final.pkl"):
    st.error("No cleaned dataset found. Please check your Python code.")
    st.stop()

# If its there, Load cleaned dataset from pickle file. `rb` = return back.
...
for col in df.columns:
    ...
    # Check if a column in a pandas DataFrame has a datetime-like data type, including timezones, and capture date range.
    # This approach for detecting dattime cols handles carious datetime formats incl. `datetime64[ns, UTC]`.
    ...
    # Otherwise treat as categorical or text and capture sample values
        ...
        unique_vals = df[col].dropna().unique() # Removes any NaN values from the column. Then extract the remaining distinct values as a NumPy array.
...
if st.button("Generate Filter Suggestions"):
    ...
    if not user_prompt.strip():
        ...
        # Display spinner msg while querying AI
        with st.spinner("AI is analyzing your data and crafting filter ideas…"):
            try:
                # Construct system prompt to explain available columns and instructions for AI - gather filter info.
                system_prompt = (
                ...
                resp = client.chat.completions.create(
                    ...
                    model="gpt-3.5-turbo",
                    messages=[
                        # Provide system instructions - response that we'll gather
            ...
            # Catch any exception that is an instance of the `Exception` class or its subclasses, and it assigns the caught exception object to the variable `e`.
            # Access details about the error, such as its message, type, and traceback, which can be useful for debugging, logging, or handling the error appropriately.
